...
public class BlackController : MonoBehaviour
{
	...
	[SerializeField] private ControllBlack controllBlack;
    ...
    private void Update()
    {
        if (!controllBlack.isBlackEnable)
        {
			BlackPlayer.transform.position = new Vector3(WhitePlayer.transform.position.x, WhitePlayer.transform.position.y, WhitePlayer.transform.position.z);
        }
    }
